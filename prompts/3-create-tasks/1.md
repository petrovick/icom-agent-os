...
--- CUSTOM INSTRUCTIONS FOR THESE TASKS ---

Spec to use:
- Use the spec at: agent-os/specs/pix-outgoing-stream-pull-api/spec.md as the single source of truth.

What I want as a result:
- A structured list of EPICS and TASKS for the "Pix Outgoing Stream Pull API".
- Tasks must focus on:
  - Refining and validating the architecture.
  - Designing contracts, data models, and operational runbooks.
  - Planning performance, reliability, security, and observability work.
- Do NOT include implementation work such as:
  - "Implement endpoint X in file Y"
  - "Write code/migrations/configuration"
- All tasks should be about design, documentation, validation, alignment, and planning.

Constraints:
- Keep tasks clear and actionable, but at an architectural/planning level.
- Provide acceptance criteria for each epic or each major task, in natural language.
- Make sure tasks explicitly cover:
  - Handling millions of requests per minute.
  - XML batching with boundaries (up to 10 messages).
  - Cursor (`pi-pull-next`) semantics and concurrency with up to 6 threads per PSP.
  - Observability, rate limiting, backpressure, and error handling.
