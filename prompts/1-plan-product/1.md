...
--- CUSTOM INSTRUCTIONS FOR THIS PRODUCT PLAN ---

Product name: "Pix Outgoing Stream Pull API"

Context:
- This product is a high-throughput, Pix-like outgoing message pull system.
- It should behave similarly to the SPI message pulling model from Banco Central do Brasil.
...
- Scale:
  - The system must support **millions of requests per minute**.
  - High availability, horizontal scalability, and partitioning/sharding are important.

VERY IMPORTANT:
- In this `plan-product` step, focus on:
  - Product vision and goals.
  - Target users/clients of this API.
  - Main use cases and constraints.
  - Success metrics and high-level risks.
- Do NOT go into implementation or detailed architecture yet.
- The output should be a clear product plan that will later guide:
  - An architectural spec.
  - A tasks/backlog document.